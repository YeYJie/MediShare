...
func checkEcho(t *testing.T, stub *shim.MockStub, name string, value string) {
	res := stub.MockInvoke("1", [][]byte{[]byte("echo"), []byte(name)})
	if res.Status != shim.OK {
		fmt.Println("Query", name, "failed", string(res.Message))
		t.FailNow()
	}
	...
	if string(res.Payload) != value {
		fmt.Println("Echo return ", string(res.Payload), " != ", name)
		t.FailNow()
	}
}
...
func TestShit(t *testing.T) {
	scc := new(SimpleAsset)
	stub := shim.NewMockStub("data", scc)

	checkInit(t, stub, nil)

	checkEcho(t, stub, "yeyongjie", "yeyongjie")
}
